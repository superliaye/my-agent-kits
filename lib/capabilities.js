...
//   capabilities/skills/@<group>/.../<name>/SKILL.md — skills may sit under one or more
//     nested grouping folders (any dir with no SKILL.md of its own, e.g. @loop/@setup/,
//     @loop/@feedback-loops/). By convention group folders at every level take an `@`
//     prefix (e.g. @my, @matt-pocock, @loop/@setup) so they read as namespaces and sort
//     apart from skills; the prefix is cosmetic, detection is purely structural. Grouping
//     is a source-organization nicety only — deploy flattens skills back to
...
const TYPE_LOCATIONS = {
  ...
  agents:       { dir: "capabilities/agents",        suffix: "",                 isDir: true  },
...
// Folder-based capability types are identified by a marker file: a folder holding
// the marker IS a capability (name = its leaf folder); a folder without one is a
// grouping namespace (recurse). Skills use SKILL.md, agents use AGENT.md.
const DIR_MARKERS = { skills: "SKILL.md", agents: "AGENT.md" };

export function listAllCapabilities() {
  const out = { instructions: [], skills: [], agents: [], plugins: [], mcp: [], hooks: [], bundles: [] };
  for (const [type, cfg] of Object.entries(TYPE_LOCATIONS)) {
    ...
    for (const entry of entries) {
      ...
      // Non-dir types yield one candidate. For skills, walk the subtree: a folder
      // with a SKILL.md is a skill (name = its folder); a folder without one is a
      // grouping folder — descend into it and repeat, to any depth, so skills may
      // nest under nested namespaces (e.g. @loop/@feedback-loops/<name>/). Leaf folder
      // names must stay globally unique — deploy flattens all skills to
      // <skillsRoot>/<name>/.
      const candidates = [];
      if (cfg.isDir) {
        if (!statSync(full).isDirectory()) continue;
        collectDirCandidates(full, entry, DIR_MARKERS[type] ?? "SKILL.md", candidates);
      } else {
      ...
      if (type === "bundles") {
        ...
        //     host-aware (~/.claude/skills/ for Claude, ~/.codex/skills/ for Codex),
        //     so no host_flag_map / per-agent loop is needed. source/pinned_commit
        //     are not used; installer.package carries the immutable GitHub
        //     commit URL and installer.skills names the reviewed skills.
...
// Collect skill candidates from a directory subtree into `out`. A directory with
// a SKILL.md is a skill (name = its leaf folder name); the walk stops there, so a
// skill's own internal subdirs (helpers/, recipes/, _unshipped/) are never mistaken
// for nested skills. A directory without a SKILL.md is a grouping folder (e.g. @loop,
// @loop/@feedback-loops) — recurse into each child. Supports arbitrarily nested
// namespaces; deploy flattens every skill to <skillsRoot>/<name>/, so nesting is a
// source-organization nicety only.
function collectDirCandidates(dir, name, marker, out) {
  const fm = readMarkerFrontmatter(join(dir, marker));
  if (fm) {
    out.push({ name, path: dir, frontmatter: fm });
    return;
  }
  for (const child of readDirSafe(dir)) {
    ...
    collectDirCandidates(childFull, child, marker, out);
  }
}

// Parse a marker file's (SKILL.md / AGENT.md) frontmatter, or return null if the
// file is absent (the signal that a directory is a grouping folder, not a leaf).
function readMarkerFrontmatter(markerFile) {
  try {
    return parseFrontmatter(readFileSync(markerFile, "utf8"));
  } catch {
    return null;
  }
}
...
// Build a name → type ("skills", "agents", …) map over every available
// capability, scanning the tree once. Lets a caller place each `+name` delta in
// the right slot without knowing its type up front, and without re-scanning per
// name. A name claimed by two types (collision) resolves to the first scanned.
export function capabilityTypeMap() {
  const all = listAllCapabilities();
  const map = new Map();
  for (const type of Object.keys(all)) {
    for (const c of all[type]) {
      if (!map.has(c.name)) map.set(c.name, type);
    }
  }
  return map;
}
