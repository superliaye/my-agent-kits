  ...
  for (const [type, cfg] of Object.entries(TYPE_LOCATIONS)) {
    ...
    for (const entry of entries) {
      ...
      if (cfg.isDir) {
        ...
        if (type === "skills") {
          collectSkillCandidates(full, entry, candidates);
        } else {
          const direct = readSkillFrontmatter(join(full, "SKILL.md"));
          if (direct) candidates.push({ name: entry, path: full, frontmatter: direct });
        }
      } else {
...
function collectSkillCandidates(dir, name, out) {
  ...
  for (const child of readDirSafe(dir)) {
    ...
    collectSkillCandidates(childFull, child, out);
  }
}
