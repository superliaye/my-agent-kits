...
// it's the "empty start" preset, useless when combined. The `experimenting-*`
// presets are *available* but not pre-checked: the superpowers plugin
...
function applyCapabilityDelta(capabilities, spec) {
  for (const tok of spec.split(",")) {
    ...
    if (op === "+") {
      const types = ["instructions", "skills", "agents", "plugins", "mcp", "hooks", "bundles"];
