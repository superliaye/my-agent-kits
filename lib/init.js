...
import { deploy, removeDeployedSkill, bundlesNeedingBunInstall, ensureBunInstalledNow } from "./deploy.js";
...
export const DEFAULT_SELECTED_PRESETS = ["engineering", "experimenting-engineering", "productivity", "feature-loop", "loop-full-swe"];
...
async function pickPreset(flags, interactive) {
  ...
  const picked = await multiselect({
    message: "Pick one or more presets (space to toggle, enter to confirm; capabilities are deduped across selections)",
    options,
  ...
  for (const tok of spec.split(",")) {
    ...
    if (op === "+") {
      ...
      for (const ptype of types) {
        if (!capabilities[ptype].includes(name)) {
          capabilities[ptype].push(name);
          break;
        }
      }
    } else if (op === "-") {
