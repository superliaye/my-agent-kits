...
const DEFAULT_SELECTED_PRESETS = ["engineering", "experimenting-engineering", "productivity", "feature-loop", "build-feature-workflow", "loop-full-swe"];

export async function runInit({ flags, repoDir, kitRoot }) {
  // `--default` takes every interactive default (the pre-checked presets, the
  // preset's agents, global scope) and applies without prompting — the
  // "enter through everything" path. Explicit flags still override individual
  // defaults (e.g. `--default --scope repo`).
  const useDefaults = Boolean(flags.default);
  // Non-interactive when --default is given, OR when all three core configs are
  // passed — either way the user has decided, no need to prompt or confirm.
  // Otherwise the wizard prompts for the missing pieces.
  const interactive = !useDefaults && !(flags.preset && flags.agents && flags.scope);
  ...
  if (useDefaults) {
    clackLog.info(`Defaults: preset=${preset.name} · agents=${agents.join(",")} · scope=${scope}`);
  }
...
// Bundles always install globally regardless of --scope (gstack has no
...
async function pickScope(flags, interactive) {
  ...
  // Default to "global" — most users land here when running the wizard for
  // ad-hoc personal setup; per-repo state is the less common case (and easy
  // to opt into explicitly with `--scope repo`).
