...
export async function runUpdate({ flags, repoDir, kitRoot }) {
  ...
  let adopted = { instructions: [], skills: [], plugins: [], mcp: [], hooks: [] };
  ...
  deploy({
    ...
    // Re-use the merge strategy the user picked at init time. Default to
    // "overwrite" for state files written before v0.5 (no field persisted).
    claudeMdMerge: state.claude_md_merge ?? "overwrite",
    isUpdate: true,
  });
...
function applyPrimitiveOps(base, adopted, removedQualified) {
  const out = JSON.parse(JSON.stringify(base ?? { instructions: [], skills: [], plugins: [], mcp: [], hooks: [] }));
