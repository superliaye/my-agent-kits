...
export async function runUpdate({ flags, kitRoot }) {
  ...
  // Resolve preset/agents/primitives exactly as `init --default` would, with the
  ...
  const primitives = preset.primitives;

  deploy({ kitRoot, preset, agents, primitives, isUpdate: true });
  ...
  if (claudeSelected && (primitives.plugins ?? []).length > 0) {
  ...
  const code = verify({ agents, primitives });
