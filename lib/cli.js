...
export function helpText() {
  return [
    ...
    "",
    "Usage:",
    "  agent-kit init [TARGET] [--default] [--preset NAME[,NAME2]] [--agents claude,codex] [--scope repo|global] [--primitives '+x,-y'] [--bundles name1,name2] [--codex-personal-layer] [--claude-md overwrite|concat|skip]",
    "  agent-kit update [TARGET] [--content-only|--adopt-preset-defaults|--dry-run]",
    ...
    "",
    "  --default   Accept every wizard default (pre-checked presets, the preset's agents, global scope)",
    "              and apply without prompting — the 'enter through everything' path. Explicit flags",
    "              still override individual defaults, e.g. `init --default --scope repo`.",
    "",
  ].join("\n");
}
