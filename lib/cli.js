...
export function helpText() {
  return [
    ...
    "",
    "Usage:",
    "  agent-kit init [--default] [--preset NAME[,NAME2] | --no-preset] [--agents claude,codex] [--capabilities '+x,-y'] [--bundles name1,name2]",
    "  agent-kit update            # interactive: adjust your current selection (pre-checked from the manifest)",
    "  agent-kit update --current  # non-interactive: re-apply the current selection at the new kit version",
    ...
    "",
    "  --no-preset Start from an empty base and pick capabilities via --capabilities (or the",
    "              interactive customize step). Cannot be combined with --preset.",
    "",
  ].join("\n");
}
