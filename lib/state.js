...
export function writeState(repoDir, state) {
  ...
  const out = {
    ...
    claude_md_merge: state.claudeMdMerge ?? "create",
    primitives_at_last_run: state.primitives,
    // bundle_commits: { gstack: <sha>, ... } — records the commit each bundle
    // was last successfully installed at. update.js compares this to the
    // current pinned_commit in the bundle file to decide whether to re-run
    // the installer.
    bundle_commits: state.bundleCommits ?? {},
  };
