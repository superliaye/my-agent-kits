...
const CAPABILITY_TYPES = ["instructions", "skills", "agents", "plugins", "mcp", "hooks", "bundles"];
...
export function loadPreset(name) {
  ...
  // Normalize every capability type to an array. Validation tolerates an omitted
  // type (e.g. a preset without `agents:`); downstream code indexes these slots
  // directly (applyCapabilityDelta, deploy), so an absent key must become [].
  for (const t of CAPABILITY_TYPES) {
    if (!Array.isArray(raw.capabilities[t])) raw.capabilities[t] = [];
  }
...
export function loadPresets(names) {
  ...
  const mergedCapabilities = { instructions: [], skills: [], agents: [], plugins: [], mcp: [], hooks: [], bundles: [] };
...
// Synthesize the "empty starting point" preset in code — the base both the
// interactive zero-selection path and the `--no-preset` flag start from. There
// is no preset file to load, so this builds the same {name, description,
// extends, default_agents, capabilities} shape loadPresets returns, with every
// capability slot present and empty (reduced over CAPABILITY_TYPES, not a
// hardcoded key list, so a new type can't silently miss a slot). Name is
// "(empty)" — it only feeds the --default log line and is never persisted.
export function emptyPreset() {
  return {
    name: "(empty)",
    description: "Empty starting point — pick capabilities in the next step",
    extends: null,
    default_agents: ["claude"],
    capabilities: Object.fromEntries(CAPABILITY_TYPES.map((t) => [t, []])),
  };
}
...
function validatePresetShape(name, p) {
  ...
  for (const t of CAPABILITY_TYPES) {
    if (p.capabilities[t] !== undefined && !Array.isArray(p.capabilities[t])) {
      throw new Error(`Preset ${name}: 'capabilities.${t}' must be an array if present`);
    }
  }
}
