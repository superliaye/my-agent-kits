...
export function normalizeBundleVerifyPaths(pathSpec) {
  if (typeof pathSpec === "string") return [pathSpec];
  if (Array.isArray(pathSpec)) return pathSpec.filter((p) => typeof p === "string");
  return [];
}

export function verify({ agents, capabilities }) {
  ...
  for (const aKey of agents) {
    ...
    const paths = a.paths.global;
    log(`  ${a.label}`);

    for (const ptype of a.supports.capabilityTypes) {
      const target = expandTarget(paths[ptype], home);
      if (!target) continue;
      // Skip capability types the user didn't actually select — e.g. preset has
      // no skills, so don't fail because .claude/skills doesn't exist.
      if (!capabilities?.[ptype] || capabilities[ptype].length === 0) continue;
  ...
  // to global agent directories.
  //
  // AGENT_KIT_SKIP_BUNDLE_INSTALL=1 (test/staging mode) skips this check
  // since deployBundle didn't actually run the installer.
  if ((capabilities?.bundles ?? []).length > 0 && process.env.AGENT_KIT_SKIP_BUNDLE_INSTALL !== "1") {
    const allBundles = (listAllCapabilities().bundles ?? []);
    ...
    for (const name of capabilities.bundles) {
      ...
      for (const aKey of agents) {
        const pathSpecs = normalizeBundleVerifyPaths(meta.verifyPaths?.[aKey]);
