...
export function verify({ repoDir, agents, scope, primitives, codexPersonalLayer }) {
  ...
  if ((primitives?.bundles ?? []).length > 0 && process.env.AGENT_KIT_SKIP_BUNDLE_INSTALL !== "1") {
    ...
    for (const name of primitives.bundles) {
      ...
      if (!meta) {
        log(`    ${name.padEnd(20)} MISSING (bundle metadata not found in kit)`);
        ok = false;
        continue;
      }
      for (const aKey of agents) {
        ...
        if (existsSync(expanded) && statSync(expanded).isDirectory() && readdirSync(expanded).length > 0) {
          log(`    ${label} ${expanded.replace(home, "~")}  OK`);
        } else {
          log(`    ${label} ${expanded.replace(home, "~")}  MISSING`);
          ok = false;
        }
      }
    }
  ...
  if (agents.includes("claude") && (primitives?.plugins ?? []).length > 0) {
    ...
    for (const name of primitives.plugins) {
      ...
      log(`    ${name.padEnd(20)} ${present ? "OK (installed)" : "MISSING (not in installed_plugins.json)"}`);
      if (!present) ok = false;
    }
  }
