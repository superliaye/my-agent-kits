...
// `--agent codex --global` to ~/.agents/skills/.
// We always specify per agent so each lands in the right host dir.
//
// "Pin" mechanism: the source spec is recorded verbatim in the manifest and
...
export function deployNpxSkillsBundle({ meta, bundleName, agents, log, priorPin }) {
  ...
  const declaredSkills = meta.installer?.skills;
  const skills = Array.isArray(declaredSkills) ? declaredSkills : ["*"];
  if (skills.length === 0 || skills.some((skill) => typeof skill !== "string" || !isSafeNpxSkillName(skill))) {
    log(`! Bundle '${bundleName}' has missing or unsafe installer.skills. Skipping.`);
    return null;
  }

  // Pin-skip: manifest already records this source spec installed; skip the
  ...
  let installSource = pkg;
  let installCwd;
  const immutableSource = parseImmutableGitHubTreePackage(pkg);
  if (immutableSource) {
    installCwd = checkoutImmutableNpxSkillsSource({
      ...immutableSource,
      bundleName,
      log,
    });
    if (!installCwd) return null;
    installSource = ".";
  }
  ...
  for (const agent of agents) {
    ...
    const args = buildNpxSkillsArgs({ packageSpec: installSource, cliAgent, skills });
    log(`> npx ${args.join(" ")}`);
...
export function buildNpxSkillsArgs({ packageSpec, cliAgent, skills }) {
  const args = ["-y", "skills", "add", packageSpec, "--global", "--agent", cliAgent];
  for (const skill of skills) args.push("--skill", skill);
  args.push("--yes");
  return args;
}

export function parseImmutableGitHubTreePackage(packageSpec) {
...
function checkoutImmutableNpxSkillsSource({ source, commit, bundleName, log }) {
  if (!isSafeNpxSkillName(bundleName)) {
    log(`! Bundle '${bundleName}' has an unsafe name. Skipping.`);
    return null;
  }

  const cacheDir = join(bundleCacheRoot(), "npx-skills", bundleName, commit);
  if (existsSync(cacheDir)) {
    const head = spawnSync("git", ["-C", cacheDir, "rev-parse", "HEAD"], { stdio: "pipe" });
    if (head.status === 0 && head.stdout.toString().trim().toLowerCase() === commit) {
      log(`✓ Bundle source cache up-to-date at ${commit.slice(0, 7)}`);
      return cacheDir;
    }
    rmSync(cacheDir, { recursive: true, force: true });
  }
  ...
  ensureDir(cacheDir);
  log(`> Fetching ${source} at ${commit.slice(0, 7)} → ${cacheDir.replace(homedir(), "~")}`);
  const commands = [
    ["init", cacheDir],
    ["-C", cacheDir, "remote", "add", "origin", source],
    ["-C", cacheDir, "fetch", "--depth", "1", "origin", commit],
    ["-C", cacheDir, "checkout", "--detach", "FETCH_HEAD"],
  ];
  for (const args of commands) {
    const result = spawnSync("git", args, { stdio: "inherit" });
    if (result.status !== 0) {
      log(`! Could not prepare immutable source ${source} at ${commit.slice(0, 7)}`);
      rmSync(cacheDir, { recursive: true, force: true });
      return null;
    }
  }
  ...
  const head = spawnSync("git", ["-C", cacheDir, "rev-parse", "HEAD"], { stdio: "pipe" });
  if (head.status !== 0 || head.stdout.toString().trim().toLowerCase() !== commit) {
    log(`! Immutable source checkout did not resolve to ${commit}`);
    rmSync(cacheDir, { recursive: true, force: true });
    return null;
  }
  return cacheDir;
...
//   @scope/pkg
//   @scope/pkg@1.2.3
//   https://github.com/owner/repo/tree/<40-char sha>
export function isSafeNpxSkillsPackage(s) {
  const packageSpec = /^@?[A-Za-z0-9][A-Za-z0-9._\-]*(\/[A-Za-z0-9][A-Za-z0-9._\-]*)?(@[A-Za-z0-9][A-Za-z0-9._\-]*)?$/;
  const immutableGitHubTree = /^https:\/\/github\.com\/[A-Za-z0-9][A-Za-z0-9._\-]*\/[A-Za-z0-9][A-Za-z0-9._\-]*\/tree\/[0-9a-fA-F]{40}$/;
  return packageSpec.test(s) || immutableGitHubTree.test(s);
}

function isSafeNpxSkillName(s) {
  return s === "*" || /^[A-Za-z0-9][A-Za-z0-9._\-]*$/.test(s);
}
