...
export function deploy({ repoDir, kitRoot, preset, agents, scope, primitives, codexPersonalLayer = false, claudeMdMerge = "create", isUpdate = false }) {
  ...
  //     locations (~/.claude/skills/gstack/, ~/.agents/skills/gstack/) regardless
...
// are excluded from the recursive copy into consumer repos, so a deployed skill
// carries only what an agent needs at use-time. Returns a cpSync `filter` that
// drops any path under an `_unshipped/` segment relative to the skill root.
