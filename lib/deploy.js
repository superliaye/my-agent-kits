...
// Top-level bundle deploy: looks up metadata, validates inputs, ensures bun,
// clones at pinned_commit, runs the installer per agent. Returns true on
// success so the caller can record it in state.
function deployBundle({ kitRoot, bundleName, agents, log }) {
  ...
  if (!meta.installer || typeof meta.installer.command !== "string") {
