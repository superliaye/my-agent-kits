...
// No APM, no apm.yml, no apm_modules.
...
export function deploy({ repoDir, kitRoot, preset, agents, primitives, isUpdate = false }) {
  const log = (line) => process.stdout.write(line + "\n");

  // 1. Copy primitives from kitRoot/.apm/ to each agent's global target locations.
  ...
  deployPrimitivesDirectly({ kitRoot, agents, primitives, log });
  ...
  if (agents.includes("claude") && (primitives.plugins ?? []).length > 0) {
    deployPlugins(kitRoot, primitives.plugins, log);
  }
  ...
  if (agents.includes("codex") && (primitives.skills ?? []).length > 0) {
    compileSkillsForCodex(primitives, log);
  }
}
...
// Copy primitives from kitRoot/.apm/ to each agent's global target locations.
// Mirrors the apm-install + apm-compile pipeline minus the wrapping. CLAUDE.md
// is handled by the caller; Codex's AGENTS.md is always overwritten here.
function deployPrimitivesDirectly({ kitRoot, agents, primitives, log }) {
...
// the codex target).
function compileSkillsForCodex(primitives, log) {
