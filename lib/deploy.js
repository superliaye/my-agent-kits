...
function deployBundle({ kitRoot, bundleName, agents, log }) {
  ...
  if (meta.installerKind === "npx-skills") {
    return deployNpxSkillsBundle({ meta, bundleName, log });
  }
...
// `npx skills add <package>` flavor (HyperFrames). The skills CLI itself
// detects installed AI hosts (Claude Code, Codex, Cursor) and writes to their
// skills dirs — so we run once, not once per agent. No git clone, no cache
// dir; npx handles fetch and execution.
//
...
function deployNpxSkillsBundle({ meta, bundleName, log }) {
  ...
  log(`> npx -y skills add ${pkg}`);
  const r = spawnSync("npx", ["-y", "skills", "add", pkg], {
    stdio: "inherit",
    shell: process.platform === "win32",
    env,
  });
  if (r.status !== 0) {
    log(`! Bundle '${bundleName}': \`npx skills add ${pkg}\` exited ${r.status}`);
    return false;
  }
  log(`✓ Bundle '${bundleName}' installed via npx skills add ${pkg}`);
  lastInstalledBundleCommits.set(bundleName, pkg);
  return true;
}
