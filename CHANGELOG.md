...
- **The shared visual-capture contract isolates each run's `agent-browser` session and keeps screenshots out of the working tree** ([capabilities/snippets/visual-env-routing.md](capabilities/snippets/visual-env-routing.md), included by `design-critic`, `product-critic`, and `loop-build-acceptance`) — a visual consumer now connects under a unique per-run `--session <name>` instead of the unnamed default, so a critic or acceptance pass running in parallel on the same machine no longer shares and clobbers one session; and it writes screenshots/scratch captures to a host-neutral scratch path (the OS temp dir or `~/.cache/...`), never the repo tree, deleting any tool-forced in-repo file before returning. Removes the stray `.design-verify-*.png` a critic left for the build agent to clean up by hand, and the cross-agent session collision latent in the parallel critique committee.
- **`/electron-visual-loop` gains troubleshooting for two failures the visual loop hit mid-run** ([capabilities/skills/@loop/@feedback-loops/electron-visual-loop/SKILL.md](capabilities/skills/@loop/@feedback-loops/electron-visual-loop/SKILL.md)) — (1) a stale cached target after switching the connected app or instance: reset by reconnecting under a fresh `--session` name (or closing only your own session), not `close --all` or a bare `close`, which act on the shared default session and can tear down another agent's agent-browser session on the same box; (2) a `screenshot` that hangs or returns blank on a minimized or 0×0 renderer: resize to a non-zero on-screen size first, or read state from computed styles / the a11y tree.
- **Research fan-out marks and verifies load-bearing claims at HEAD so the planner stops re-reading the same source** ([capabilities/snippets/research-fan-out.md](capabilities/snippets/research-fan-out.md), shared by `/loop-plan-semiauto` and `/loop-plan-manual`) — a research sub-agent now verifies any claim the plan's scope hinges on ("this part is already built", "this endpoint exists") against live source at HEAD and returns the `file:line` + verbatim line it saw, marked `[verified@HEAD]`, so the resident trusts a verified claim instead of re-confirming it by hand and re-checks only unmarked ones; the problem-area map also asks for the precise call sites the change will touch and the tests that lock them where the research already reveals them. Addresses friction flagged in two consecutive retros.
- **The build agent runs critique and review as explicit ordered steps, with a re-review backstop** ([capabilities/agents/@loop/loop-build-agent/AGENT.md](capabilities/agents/@loop/loop-build-agent/AGENT.md)) — critique and review are now stated as separate, ordered steps (critique first, land its fixes, then review) so the review committee judges the critique-shaped code rather than the pre-critique code; and if a substantial change lands after review already ran, the agent re-runs the review committee, the same way it re-validates acceptance after a regressing fix. Closes a gap where spawning critique and review in one parallel batch left a critique-driven redesign un-reviewed.
...
- **design-critic + product-critic agents** ([capabilities/agents/@reviews/design-critic/](capabilities/agents/@reviews/design-critic/), [capabilities/agents/@reviews/product-critic/](capabilities/agents/@reviews/product-critic/)) — findings-only critics in the `@reviews` family. design-critic works only from the rendered pixels — it captures and judges the running UI (or a handed-over screenshot) and stops to ask for a render rather than guessing from markup; product-critic works only from the running product — it walks the live flow and stops to ask for a reachable UI rather than guessing from code. Both end with the shared critique-finding-contract.
