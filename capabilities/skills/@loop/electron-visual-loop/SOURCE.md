...
- Dropped upstream's `name: electron` field — the kit's capability loader derives the name from the folder (`electron-visual-loop`).
...
- **Opt-in only** — not wired into any preset. Install per-repo with `agent-kit init … --capabilities '+electron-visual-loop'`, or add to a preset's `capabilities.skills` list locally if you maintain a stack-specific preset.
