...
# duplicated body in CLAUDE.md from naive concat).
core_count=$(grep -c "^# Core Instructions$" "$WORK/CLAUDE.md" 2>/dev/null || echo 0)
if [ "$core_count" = "1" ]; then
  ok "core instruction deduped across presets (one occurrence in CLAUDE.md)"
else
  fail "core instruction not deduped (found $core_count occurrences of '# Core Instructions' header in CLAUDE.md)"
fi
...
assert_content_contains "$WORK/CLAUDE.md" "TypeScript Rules" "engineering's typescript instruction in CLAUDE.md (union, not intersection)"
