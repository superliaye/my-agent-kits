...
# First init: loop preset deploys the loop-build skill.
"$KIT_ROOT/bin/agent-kit" init --preset loop --agents codex \
  || { fail "first init exited non-zero"; exit 1; }
assert_dir_exists "$HOME/.agents/skills/loop-build" "loop-build deployed by first init"

# Second init: engineering preset does NOT include loop-build.
...
if [ -d "$HOME/.agents/skills/loop-build" ]; then
  ...
  ok "init reconciled the dropped skill (loop-build removed)"
fi
...
if grep -q "loop-build" "$HOME/.agent-kit/manifest.json"; then
