...
# Global-only: `update` is a pure replay of the install manifest that `init`
# wrote (~/.agent-kit/manifest.json) — it takes no selection flags. Isolate BOTH
# HOME and USERPROFILE (node's os.homedir() reads USERPROFILE on Windows) so the
# manifest + global writes land in a throwaway dir, never the dev's real
# ~/.codex or ~/.agents.
...
# and the manifest records codex-only, so it skips the refresh entirely.
export AGENT_KIT_SKIP_PLUGIN_INSTALL=1 AGENT_KIT_SKIP_BUNDLE_INSTALL=1

# Seed a global install. Both flags present => init runs non-interactively and
# returns verify()'s code; a clean deploy yields 0. This also writes the manifest
# (engineering/codex) that the later `update` replays.
...
# --- Re-deploy via `update --current` (non-interactive replay) ---------------
# --current re-applies the manifest selection init wrote, no prompts.
...
# Global-only negative: state lives in ~/.agent-kit/, never as a per-repo file.
if [ -f "$WORK/.agent-kit.yaml" ]; then fail "update wrote $WORK/.agent-kit.yaml (per-repo state file must not exist)"; else ok "no per-repo .agent-kit.yaml written"; fi
# And the durable manifest persists in HOME after update.
assert_file_exists "$HOME/.agent-kit/manifest.json" "install manifest persists after update"
